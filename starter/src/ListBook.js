...
const ListBook = ({ listBook,reload,setReload }) => {
  const renderBookByShelf = (type) => {
    ...
    return (
      <div>
      {type === 'currentlyReading' && <h2 className="bookshelf-title">Currently Reading</h2>}
      {type === 'wantToRead' && <h2 className="bookshelf-title">Want to Read</h2>}
      {type === 'read' && <h2 className="bookshelf-title">Currently Reading</h2>}
